...
void dySky::hybrid_compute_view_recursively_md(Config* cfg, int niveau, vector<Point> &dataset, vector<Order> spo, bool* cSkyline){
...
void dySky::hybrid_compute_view_1d(Config* cfg, vector<Point> &dataset, Order o, bool* cSkyline){
...
void dySky::compute_skyline_wrt_missing_spos(Config* cfg, vector<vector<Order>> missing_spos, bool* cSkyline){
...
vector<id> dySky::hybrid_compute_skyline(Config* cfg, vector<vector<Order>> preference_cross){
	cout << "dySky::compute_skyline" <<endl;
...
void dySky::views_selection(Config* cfg, uint64_t max_storage){
...
int knapSack(int W, int wt[], int val[], int n, vector<int> &spo_ids) 
{ 
int i, w; 
int K[n+1][W+1]; 
bool keep[n+1][W+1];
